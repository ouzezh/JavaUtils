...
public class EasyExcelUtil {
    ...
    public static void main(String[] args) {
        ...
        simpleWrite(pathName, MyExcelModel.class, data);
        // 简单读
        List<MyExcelModel> list = simpleRead(pathName, MyExcelModel.class);
        StaticLog.info(list.stream().map(JSON::toJSONString).collect(Collectors.joining("\r\n", "\r\n", "")));
        // 不创建对象读
    ...
    private static <T> List<T> simpleRead(String pathName, Class<T> head) {
        return EasyExcel.read(pathName).autoTrim(true).sheet().head(head).doReadSync();
    }

    public static <T> void simpleWrite(String pathName, Class<T> clazz, List<T> data) {
        try(ExcelWriter w = EasyExcel.write(pathName, clazz).autoTrim(true).build()) {
...
//        EasyExcel.read(pathName, listener).sheet().doRead();
    ...
    private static WriteCellStyle getDefaultCellStyle(boolean isHead) {
